...
(function (ROOT, struct, undefined) {
  ...
  function isArrayLike(elem) {
    ...
    return (
      ...
      tp != "string" &&
      (elem.length === 0 ||
        (elem.length && (elem.nodeType == 1 || elem.length - 1 in elem)))
    );
  }
  ...
  struct.prototype = {
    ...
    init: function (config) {
      ...
      addListener(
        ...
        STARTEVENT.join(" ") +
          " click" +
          (this.mousewheel ? " mousewheel DOMMouseScroll" : ""),
        handler
      );
      addListener(
        DOC,
      ...
      console.log("**this.pageData.container : ", this.pageData.container);
    ...
    resize: function () {
      ...
      this.size = this.getSize(
        (this.offsetParent =
          this.container[
            pst == "absolute" || pst == "fixed" ? "offsetParent" : "parentNode"
          ] || DOC.body)
      );
    ...
    slide: function (_index) {
      var self = this,
        ...
        curPos =
          parseFloat(getStyle(this.container, dir ? "top" : "left")) || 0,
        type = dir ? "top" : "left",
        css = {},
        tarPos;
      ...
      this.container.children[current].className = "hidden";
      this.container.children[index].className = "display";
        ...
        case 2:
          if (canDrag && this.rect) {
            ...
            if (this.drag == null && _rect.toString() != rect.toString()) {
              this.drag =
                Math.abs(offset) >= Math.abs(rect[1 - dir] - _rect[1 - dir]);
            ...
            if (this.drag) {
              ...
              this.container.style[dir ? "top" : "left"] =
          ...
          if (canDrag) {
            ...
            if (ev.length && (ev.eventCode == 1 || this.drag)) {
              ...
              if (isDrag) {
                ...
                while (
                  sub * (curPos - tarPos) >
                    this.getOuterSize(this.pages[index]) / 2 &&
    ...
    getPos: function (index) {
      var type = this.direction ? "Top" : "Left",
        ...
        sum =
          this.getSum(0, index) +
          this["getMargin" + type + "Size"](this.container) +
          this["getBorder" + type + "Size"](this.container);
    ...
    getOuterSize: function (elem, withMargin) {
      return (
        elem[this.direction ? "offsetHeight" : "offsetWidth"] +
  ...
  each("margin padding border".split(" "), function (type) {
    ...
    struct.prototype[camelCase("get-" + type) + "Size"] = function (elem) {
      return (
        this[
          camelCase("get-" + type) + (this.direction ? "Top" : "Left") + "Size"
        ](elem) +
        this[
          camelCase("get-" + type) +
